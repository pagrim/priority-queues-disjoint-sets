...
class TableMerge:
    ...
    def merge(self, destination, source):
        ...
        logging.debug('Calculated data rows indices as destination %d source %d', dest_rows_idx, src_rows_idx)
        if dest_rows_idx != src_rows_idx:
            ...
            if merged_root_idx == src_rows_idx:
                merged_non_root_idx = dest_rows_idx
            else:
                merged_non_root_idx = src_rows_idx
                merged_root_idx = dest_rows_idx
            self.row_counts[merged_root_idx] += self.row_counts[merged_non_root_idx]
            self.row_counts[merged_non_root_idx] = 0
